...
function renderTrangThai($value)
{
    switch ((int)$value) {
        case 1:
            return '<span class="badge badge-coc">📌 Đã cọc</span>';
        case 2:
            return '<span class="badge badge-ban">✅ Đã bán</span>';
        default:
            return '<span class="badge badge-chua">🕓 Chưa bán</span>';
    }
}
?>
    ...
    <div class="grid">
        ...
        <?php else: ?>
            <?php foreach ($list as $row): ?>
                ...
                <?php
                $isNha = ($row['loai_bds'] ?? '') === 'nha';

                // LINK
                $link_view = $isNha
                    ? "index.php?action=nha_detail&id={$row['id']}"
                    : "index.php?action=dat_detail&id={$row['id']}";
                ...
                <div class="card">
                    ...
                    <div class="actions">
                        ...
                        <?php if (
                            ...
                            $_SESSION['user']['id'] == ($row['user_id'] ?? 0)
                        ): ?>
                            <a href="<?= $link_edit ?>" class="btn-edit">Sửa</a>
                            <a href="index.php?action=baidang_delete&id=<?= $row['id'] ?>"
                               class="btn-delete"
                               onclick="return confirm('Bạn có chắc muốn xoá bài này?')">
                                Xoá
                            </a>
                        <?php endif; ?>
                    </div>
